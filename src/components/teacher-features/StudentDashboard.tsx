...
import {
  ...
  Sun, Moon, RefreshCw, FileText, Video, Share, Camera, Shield
} from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      if (status === 'authenticated') {
        try {
          const response = await fetch(`/api/user/students?role=${filterRole}`);
          if (response.ok) {
            const data = await response.json();
            ...
            <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl border p-4 mb-6`}>
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <select
                  ...
                  className={`px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none ${
                    darkMode ? 'bg-gray-700 border-gray-600 text-gray-100' : 'bg-white border-gray-300 text-gray-900'
                  }`}
                >
            ...
            <div className="space-y-4">
              {filteredStudents.length === 0 ? (
                <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl border-2 p-12 text-center`}>
                  ...
                  <h3 className={`text-xl font-bold ${darkMode ? 'text-gray-200' : 'text-gray-900'} mb-2`}>No Students Found</h3>
                ...
                filteredStudents.map((student) => (
                  <div key={student.id} className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl border-2 p-6 hover:shadow-xl transition-all`}>
                    <div className="flex flex-col lg:flex-row items-start gap-6">
                      {/* Avatar */}
                      <div className="w-20 h-20 rounded-xl overflow-hidden bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center flex-shrink-0 shadow-lg">
                        {student.avatar ? (
                          <img src={student.avatar} alt={student.name} className="w-full h-full object-cover" />
                        ) : (
                          <span className="text-white font-bold text-2xl">{getInitials(student.name)}</span>
                        )}
                      </div>
                      ...
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-3 mb-3">
                          <h3 className={`text-xl font-bold ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>{student.name}</h3>
                        ...
                        {student.bio && (
                          <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-3 line-clamp-2`}>{student.bio}</p>
                        )}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                          {student.qualification && (
                            <div className={`${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg p-3`}>
                              ...
                              <p className={`text-sm font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'} truncate`}>{student.qualification}</p>
                            </div>
                          )}
                          {student.experience && (
                            <div className={`${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg p-3`}>
                              <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-1`}>Experience</p>
                              <p className={`text-sm font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'} truncate`}>{student.experience}</p>
                            </div>
                          )}
                          {student.subjects && (
                            <div className={`${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg p-3 sm:col-span-2`}>
                              ...
                              <p className={`text-sm font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'} truncate`}>{student.subjects}</p>
                            </div>
                          )}
                        </div>
                        ...
                        {(student.website || student.linkedin || student.twitter || student.instagram) && (
                          <div className="flex gap-2 mt-4">
                            {student.website && (
                              <a href={student.website} target="_blank" rel="noopener noreferrer" className={`w-8 h-8 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'} rounded-lg flex items-center justify-center hover:scale-110 transition-all`}>
                                <Globe className="w-4 h-4" />
                              </a>
                            )}
                            {student.linkedin && (
                              <a href={student.linkedin} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center hover:scale-110 transition-all">
                                <Linkedin className="w-4 h-4" />
                              </a>
                            )}
                            {student.twitter && (
                              <a href={student.twitter} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-blue-100 text-blue-400 rounded-lg flex items-center justify-center hover:scale-110 transition-all">
                                <Twitter className="w-4 h-4" />
                              </a>
                            )}
                            {student.instagram && (
                              <a href={student.instagram} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-pink-100 text-pink-600 rounded-lg flex items-center justify-center hover:scale-110 transition-all">
          ...
          <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto`}>
            <div className={`p-6 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'} flex items-center justify-between sticky top-0 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-xl overflow-hidden bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center">
                  {selectedStudent.avatar ? (
                    <img src={selectedStudent.avatar} alt={selectedStudent.name} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-white font-bold text-2xl">{getInitials(selectedStudent.name)}</span>
                  )}
                </div>
                <div>
                  <h3 className={`text-2xl font-bold ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>{selectedStudent.name}</h3>
                  <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{selectedStudent.email}</p>
                </div>
              </div>
              <button
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {selectedStudent.phone && (
                    <div>
                      ...
                      <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{selectedStudent.phone}</p>
                    </div>
                  )}
                  {selectedStudent.location && (
                    <div>
                      <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Location</p>
                      <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{selectedStudent.location}</p>
                    </div>
                  )}
                  {selectedStudent.dateOfBirth && (
                    <div>
                      <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Date of Birth</p>
                      <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{formatDate(selectedStudent.dateOfBirth)} (Age: {calculateAge(selectedStudent.dateOfBirth)})</p>
                    </div>
                  )}
                  <div>
                    <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Member Since</p>
                    <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{formatDate(selectedStudent.createdAt)}</p>
                  </div>
                </div>
                {selectedStudent.bio && (
                  <div className="mt-4">
                    <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-2`}>Bio</p>
                    <p className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{selectedStudent.bio}</p>
                  </div>
                )}
              </div>
                ...
                <div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {selectedStudent.qualification && (
                      <div>
                        ...
                        <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{selectedStudent.qualification}</p>
                      </div>
                    )}
                    {selectedStudent.experience && (
                      <div>
                        <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Experience</p>
                        <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{selectedStudent.experience}</p>
                      </div>
                    )}
                    {selectedStudent.subjects && (
                      <div className="md:col-span-2">
                        <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Subjects</p>
                        <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{selectedStudent.subjects}</p>
                      </div>
                    )}
                    {selectedStudent.specialization && (
                      <div className="md:col-span-2">
                        <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Specialization</p>
                        <p className={`font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>{selectedStudent.specialization}</p>
                      </div>
                    )}
                  </div>
                  {selectedStudent.teachingStyle && (
                    <div className="mt-4">
                      ...
                      <p className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{selectedStudent.teachingStyle}</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    {selectedStudent.website && (
                      <a href={selectedStudent.website} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 rounded-lg transition-all flex items-center gap-2">
                        <Globe className="w-4 h-4" />
                        Website
                      </a>
                    )}
                    {selectedStudent.linkedin && (
                      <a href={selectedStudent.linkedin} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-lg transition-all flex items-center gap-2">
                        <Linkedin className="w-4 h-4" />
                        LinkedIn
                      </a>
                    )}
                    {selectedStudent.twitter && (
                      <a href={selectedStudent.twitter} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-blue-100 hover:bg-blue-200 text-blue-400 rounded-lg transition-all flex items-center gap-2">
                        <Twitter className="w-4 h-4" />
                        Twitter
                      </a>
                    )}
                    {selectedStudent.instagram && (
                      <a href={selectedStudent.instagram} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-pink-100 hover:bg-pink-200 text-pink-600 rounded-lg transition-all flex items-center gap-2">
                        <Camera className="w-4 h-4" />
                        Instagram
                      </a>
                    )}
