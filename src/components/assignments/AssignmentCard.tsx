...
import {
  FileText, Calendar, Clock, Users, Download, Search,
  Loader, X, Upload, CheckCircle, Eye, MessageSquare,
  TrendingUp, Award, RefreshCw, Send, ThumbsUp, Trash2,
  BookOpen, Activity, Paperclip, ExternalLink, User,
    ...
    try {
      ...
      setAllAssignments((prev) =>
        prev.map((a) =>
          a.mySubmission?.id !== submissionId
            ? a
            : { ...a, mySubmission: { ...a.mySubmission!, isCompleted: false, status: 'pending' } }
        )
      );
    }
  };

  const handleSubmitSuccess = (assignmentId: string, submission: Assignment['mySubmission']) => {
    setAllAssignments((prev) =>
    ...
    <div className={`p-3 sm:p-5 lg:p-8 min-h-screen transition-colors ${dm ? 'bg-gray-900' : 'bg-gradient-to-br from-gray-50 to-gray-100'}`}>
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5 sm:mb-6">
          <div>
            ...
            <p className={`text-sm sm:text-base ${dm ? 'text-gray-400' : 'text-gray-600'}`}>
              View, submit, and collaborate on your assignments
            </p>
          </div>
          <button
            onClick={() => fetchAssignments(true)}
            disabled={refreshing}
            className={`self-start sm:self-auto px-3 sm:px-4 py-2 border-2 rounded-xl transition flex items-center gap-2 font-semibold text-sm sm:text-base disabled:opacity-60 ${
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          {[
            {
              icon: BookOpen,
              iconBg: dm ? 'bg-blue-900' : 'bg-blue-100',
            ...
            {
              icon: CheckCircle,
              iconBg: dm ? 'bg-green-900' : 'bg-green-100',
            ...
            {
              icon: Clock,
              iconBg: dm ? 'bg-orange-900' : 'bg-orange-100',
            ...
            {
              icon: Activity,
              iconBg: dm ? 'bg-purple-900' : 'bg-purple-100',
            ...
            <div
              key={i}
              className={`rounded-xl sm:rounded-2xl p-3 sm:p-5 lg:p-6 border-2 hover:shadow-xl transition-all ${
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-9 sm:pl-10 pr-8 py-2.5 sm:py-3 border-2 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all text-sm sm:text-base ${
      ...
      {filteredAssignments.length > 0 && (
        <p className={`text-xs mb-4 ${dm ? 'text-gray-500' : 'text-gray-400'}`}>
          Showing{' '}
          <span className={`font-semibold ${dm ? 'text-gray-300' : 'text-gray-600'}`}>{filteredAssignments.length}</span>
          {' '}of{' '}
          <span className={`font-semibold ${dm ? 'text-gray-300' : 'text-gray-600'}`}>{totalAssignments}</span>
          ...
          <div className={`border-2 rounded-xl p-3 sm:p-4 mb-4 ${dm ? 'bg-blue-900/30 border-blue-700' : 'bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200'}`}>
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className={`flex items-center gap-1.5 text-xs sm:text-sm ${dm ? 'text-blue-300' : 'text-blue-700'}`}>
                ...
                {assignment.mySubmission.remarks && (
                  <p className={`text-xs sm:text-sm mt-2 italic ${dm ? 'text-blue-200' : 'text-blue-800'}`}>
                    "{assignment.mySubmission.remarks}"
                  </p>
                )}
              </div>
...
function SubmitAssignmentModal({
  ...
  assignment: Assignment;
  onClose: () => void;
  onSuccess: (assignmentId: string, submission: Assignment['mySubmission']) => void;
  darkMode: boolean;
}) {
  ...
  const { startUpload, isUploading } = useUploadThing('submissionFile', {
    onClientUploadComplete: (res) => {
      if (res?.[0]) {
        const f = res[0];
        setUploadedFile({ url: f.url, name: f.name, size: `${(f.size / 1024 / 1024).toFixed(2)} MB` });
        setUploading(false);
      }
    },
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className={`rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden shadow-2xl flex flex-col ${dm ? 'bg-gray-900' : 'bg-white'}`}>
        ...
        <div className="p-4 sm:p-6 space-y-5 overflow-y-auto flex-1">
            ...
            {uploadedFile ? (
              <div className={`border-2 rounded-xl p-3 sm:p-4 flex items-center justify-between ${dm ? 'border-green-700 bg-green-900/30' : 'border-green-300 bg-green-50'}`}>
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${dm ? 'bg-green-800' : 'bg-green-100'}`}>
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition flex-shrink-0 ${dm ? 'hover:bg-red-900/40' : 'hover:bg-red-100'}`}
                >
                  <Trash2 className="w-4 h-4 text-red-500" />
                </button>
              </div>
            ) : (
              <label className={`border-2 border-dashed rounded-xl p-6 sm:p-8 text-center hover:border-indigo-500 transition cursor-pointer block ${dm ? 'border-gray-600' : 'border-gray-300'}`}>
                <input
                  type="file"
                ...
                {uploading || isUploading ? (
                  <>
                    <Loader className="w-10 h-10 text-indigo-500 animate-spin mx-auto mb-2" />
                    <p className="text-indigo-500 font-medium text-sm">Uploading...</p>
                  </>
                ) : (
                  <>
                    <Upload className={`w-10 h-10 mx-auto mb-2 ${dm ? 'text-gray-500' : 'text-gray-400'}`} />
                    <p className={`mb-1 font-medium text-sm ${dm ? 'text-gray-300' : 'text-gray-600'}`}>Click to upload your solution PDF</p>
          ...
          <div>
            <label className={`block text-sm font-semibold mb-2 ${dm ? 'text-gray-300' : 'text-gray-700'}`}>
              Remarks (Optional)
            </label>
            <textarea
              rows={3}
          ...
          <div className={`border-2 rounded-xl p-3 sm:p-4 ${dm ? 'bg-indigo-900/30 border-indigo-700' : 'bg-indigo-50 border-indigo-200'}`}>
            <div className="flex items-start gap-2 sm:gap-3">
              <Sparkles className={`w-4 h-4 mt-0.5 flex-shrink-0 ${dm ? 'text-indigo-400' : 'text-indigo-600'}`} />
              <div>
        ...
        <div className={`p-4 sm:p-6 border-t-2 flex items-center justify-end gap-2 sm:gap-3 flex-shrink-0 ${dm ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50'}`}>
          <button
            onClick={onClose}
            className={`px-4 sm:px-6 py-2.5 border-2 rounded-xl transition font-semibold text-sm ${
              dm ? 'border-gray-600 text-gray-300 hover:bg-gray-700' : 'border-gray-300 hover:bg-gray-100'
            }`}
          >
            Cancel
          </button>
          <button
            ...
            className="px-4 sm:px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl hover:shadow-lg transition font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-sm"
          >
            {submitting
...
function CommentsModal({
  ...
  const fetchComments = useCallback(async () => {
    try {
      const res = await fetch(`/api/assignments/comments?assignmentId=${assignment.id}`);
      const data = await res.json();
  ...
  const handlePostComment = async () => {
    ...
    setPosting(true);
    try {
      const res = await fetch('/api/assignments/comments', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          assignmentId: assignment.id,
  ...
  const handleDelete = async (commentId: string) => {
    ...
    try {
      const res = await fetch(`/api/assignments/comments?id=${commentId}`, { method: 'DELETE' });
      const data = await res.json();
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className={`rounded-t-2xl sm:rounded-2xl w-full sm:max-w-3xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden shadow-2xl flex flex-col ${dm ? 'bg-gray-900' : 'bg-white'}`}>
        ...
        <div className={`p-4 sm:p-6 border-b-2 flex-shrink-0 ${dm ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gradient-to-r from-indigo-50 to-purple-50'}`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-xs sm:text-sm mt-0.5 truncate max-w-[240px] sm:max-w-full ${dm ? 'text-gray-400' : 'text-gray-600'}`}>
                {assignment.title}
              </p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6">
          {loading ? (
            ...
            <div className="text-center py-10 sm:py-12">
              <MessageSquare className={`w-12 h-12 mx-auto mb-4 ${dm ? 'text-gray-600' : 'text-gray-400'}`} />
              <p className={`font-semibold mb-1 ${dm ? 'text-gray-300' : 'text-gray-600'}`}>No comments yet</p>
                ...
                <div key={comment.id} className={`rounded-xl p-3 sm:p-4 border-2 ${dm ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-sm sm:text-base mb-2 whitespace-pre-wrap break-words ${dm ? 'text-gray-300' : 'text-gray-700'}`}>
                        {comment.content}
                      </p>
                      {comment.fileUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 border-2 rounded-lg transition text-xs font-semibold ${
        ...
        <div className={`p-3 sm:p-6 border-t-2 flex-shrink-0 ${dm ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50'}`}>
          {uploadedFile && (
            <div className={`mb-2 sm:mb-3 flex items-center gap-2 border-2 rounded-lg p-2.5 ${dm ? 'bg-gray-700 border-indigo-700' : 'bg-white border-indigo-200'}`}>
          ...
          <div className="flex gap-2">
            <textarea
              ...
              className={`flex-1 px-3 sm:px-4 py-2.5 border-2 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none text-sm sm:text-base ${
                dm ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-500' : 'border-gray-200 text-gray-900'
              }`}
            />
            <div className="flex flex-col gap-2">
              <label className={`p-2.5 sm:p-3 border-2 rounded-xl transition cursor-pointer flex items-center justify-center ${dm ? 'border-gray-600 hover:bg-gray-700' : 'border-gray-300 hover:bg-gray-100'}`}>
                ...
                {uploading || isUploading
                  ...
                  : <Paperclip className={`w-4 h-4 ${dm ? 'text-gray-400' : 'text-gray-600'}`} />}
              </label>
              <button
